 ...
 * I/P Arguments: gpioPins_et RS: Pin where RS is connected 
                  ...
                  gpioPins_et B0: Pin where B0 is connected (P_NC if not connected for 4-bit mode)  
                  gpioPins_et B1: Pin where B1 is connected (P_NC if not connected for 4-bit mode)  
                  gpioPins_et B2: Pin where B2 is connected (P_NC if not connected for 4-bit mode)  
                  gpioPins_et B3: Pin where B3 is connected (P_NC if not connected for 4-bit mode) 
                  gpioPins_et B4: Pin where B4 is connected 
                  gpioPins_et B5: Pin where B5 is connected 
                  gpioPins_et B6: Pin where B6 is connected 
                  gpioPins_et B7: Pin where B7 is connected 
 * Return value    : none
               ...
               gpioPins_et B7 )
{
  /* Copy the PIN numbers where the LCD is connected */
    ...
    GPIO_PinDirection(B4,OUTPUT);
    GPIO_PinDirection(B5,OUTPUT);
    GPIO_PinDirection(B6,OUTPUT);
    GPIO_PinDirection(B7,OUTPUT);  
}
...
void LCD_ScrollMessage(uint8_t v_lineNumber_u8, char *ptr_msgPointer_u8)
{
    ...
    {      
        ...
        _delay_ms(C_ScrollDelayTime_U8);
    }
...
static void lcd_BusyCheck(void)
{
  ...
  if(LCDConfig.RW != P_NC)                         //Perform Busy check if RW pin is used
   {
    GPIO_PinDirection(LCDConfig.B7,INPUT); // Configure busy pin as input
    ...
    do
    {
        ...
        _delay_us(10);
        busyflag = GPIO_PinRead(LCDConfig.B7);
    ...
    GPIO_PinDirection(LCDConfig.B7,OUTPUT); // Configure busy pin as Output
  }
else
 {
...
static void lcd_SendHigherNibble(uint8_t dataByte)
{
    GPIO_PinWrite(LCDConfig.B4,util_IsBitSet(dataByte,4));
    GPIO_PinWrite(LCDConfig.B5,util_IsBitSet(dataByte,5));
    GPIO_PinWrite(LCDConfig.B6,util_IsBitSet(dataByte,6));
...
static void lcd_SendLowerNibble(uint8_t dataByte)
{
    GPIO_PinWrite(LCDConfig.B0,util_IsBitSet(dataByte,0));
    GPIO_PinWrite(LCDConfig.B1,util_IsBitSet(dataByte,1));
